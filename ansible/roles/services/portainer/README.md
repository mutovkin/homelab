...
| Date | From → To | Notes |
| ---- | --------- | ----- |
| 2026-09-20 | **2.45.0 → 2.45.1** on both hosts (#282) | The one real version bump of the three adopted in #282 (telegraf and postgres were rebuilds at the same version). Watchtower reported `0cd22f754ac5`; that digest also carries `2.45.1`, `latest` and — currently — `sts`, so LTS and STS are the same build this round. Registry had not drifted: notified == local tag == registry at 19:25Z and again at 19:27Z immediately before the apply. **2.45.1 is a security release**, which is why it was taken promptly on a container holding a read-write docker socket: SSRF hardening on outbound requests (Helm chart resolution and Git HTTP/HTTPS now go through an SSRF-aware transport), certificate-based auth for Azure Blob backups disabled under FIPS, and **six** CVEs patched via dependency bumps that reach a server-only deployment like ours — `golang.org/x/mod` 0.40.0 (CVE-2026-56865, -56864), `golang.org/x/crypto` 0.56.0 (CVE-2026-56854, -78662, -56855) and `grpc` 1.83.2 (CVE-2026-84304) — plus Swarm networking and registry-credential fixes. The release names ten CVEs in total; the other four (kubectl v1.37.0 CVE-2026-39821, helm v4.2.4 CVE-2026-46600, Alpine openssl/libcurl/jq CVE-2026-63073 and CVE-2026-75803) are in the **kubectl shell image**, which this deployment does not use — counted separately rather than folded into one headline number. None of the known issues in the notes reach us (they are Async Edge and Podman environments; we run neither). **This bump MIGRATED the database, one way, on both hosts** — the fact that most changes the rollback story, and the one `/api/status` cannot tell you. The container log is explicit: `Backing up database from=/data/portainer.db to=/data/backups/portainer.db.bak`, then `migrating database from version 2.45.0 to 2.45.1`, `refreshing RBAC roles`, `refreshing user authorizations`, `DB migrated to_version=2.45.1`. **So reverting to image `18750221de87` does NOT revert the state**: the 2.45.0 binary would meet a BoltDB stamped 2.45.1. The real rollback artifact is Portainer's own pre-migration copy, `/data/portainer/backups/portainer.db.bak` (eq12_docker 1,048,576 B, n5pro_docker 131,072 B, both written 19:27:3xZ) — it is a **single slot and the next upgrade's migration overwrites it**, so an image rollback must be paired with restoring that file, and it must be copied aside first if it is to survive. Always `docker logs portainer | grep -i migrat` before calling a Portainer bump verified. **Verified 2026-09-20 19:27–19:28Z on both hosts:** `/api/status` reports `"Version":"2.45.1"`, and the `InstanceID` is unchanged on each host (`aad44a30…` on eq12_docker, `8d2c1ef2…` on n5pro_docker). Be honest about what that second one proves: `InstanceID` lives in `portainer.db` on a bind mount, so it survives almost anything short of deleting the mount — it would look identical after a failed migration or with every user orphaned. It rules out a fresh install and nothing more, and `/api/status` is an unauthenticated handler that touches neither the docker endpoint nor auth. What it does NOT cover — an authenticated `POST /api/auth` against a real account, and `GET /api/endpoints` showing `Status: 1` with a snapshot newer than the restart — is the check that would actually exercise the RBAC and user-authorization refresh this migration performed. The post-init migration line (`executing post init migration for environment endpoint_id=3`) appears on **eq12_docker only** — n5pro_docker's log has no post-init entry, which is consistent with the two hosts managing different environments but was not chased down. The authenticated path was not probed on either host; treat it as unverified until someone does. The `inet portainer_fw` allowlist table is present on both hosts after the recreate, so the published `:9000` did not silently reopen. **Rollback — the strongest of the three adopted in #282, because this one was a real version bump:** `portainer/portainer-ee:2.45.0` still resolves upstream to `sha256:18750221de87…`, verified against the superseded local image, so it is a genuine pullable reference and satisfies `docker_host`'s prune assumption ("an immutable upstream tag whose digest was verified against the running image"). The local copy on both hosts is now dangling and unreferenced and will be reaped by the next `docker_prune` (#276/#281), which is fine precisely because the tag survives it — unlike the rebuild-class entries in the observability and postgresql logs (#284). |
