...
patches under the same version tags. Map the digest back through the repository's tag list
(`hub.docker.com/v2/repositories/<repo>/tags`, match on `digest`) — if it also carries
`1.40.0`, the bump is a rebuild and the honest row says so. Measured on 2026-09-20 (#282),
where two of three "updates" were rebuilds at the running version.
