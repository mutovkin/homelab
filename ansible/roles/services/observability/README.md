...
**And resolve the digest to a VERSION before writing a row.** A new digest on a floating
tag does not imply a new application version: official images are rebuilt for base-layer
...
| Date | From → To | Notes |
| ---- | --------- | ----- |
| 2026-09-17 | victoria-metrics v1.150.0 → **v1.152.0**; grafana 13.2.0 → **13.2.2**; telegraf 1.39.3 → **1.40.0** (#275) | Watchtower reported v1.151.0 / 13.2.1 / telegraf 1.40.0; the apply landed **v1.152.0** and **13.2.2** because `:latest` had moved again upstream (registry `tag_last_pushed`: VM v1.152.0 on 09-14, grafana 13.2.2 on 09-15) — see the warning above. Both extra deltas were reviewed after the fact and carry no removals: **VM 1.151+1.152** carry **no `vm_*` removals or renames and no MetricsQL changes** — which matters because 22 `lag()` and 27 `last_over_time()` uses back every absence rule. Their vmauth JWT `match_claims` and vmselect `-deleteAuthKey` fixes do not reach us (we run vmsingle, and neither component is deployed). **But 1.151.0's headline SECURITY item DOES land on us and a first pass wrongly filed it under "not deployed here":** vmsingle now checks `-httpAuth.*` Basic Auth on request paths that do not verify a corresponding `*AuthKey` on their own — previously a path ending `/config` or `/reload` (e.g. `/api/v1/import/prometheus/config`) could reach the ingestion handler unauthenticated. `compose.yaml` runs vmsingle with exactly `--httpAuth.username` / `--httpAuth.password`, so this tightens a real surface. Nothing broke because every client already sends Basic auth (telegraf and the native agents via `outputs.http`, Grafana via the datasource, `truenas_poller.py`) and the container healthcheck uses `/health`; the post-bump ingest checks below are what proves it. **Grafana 13.2.1+13.2.2** are security and bugfix only (CVE-2026-12704, -14199, -15815, -76154, -79656). The one provisioning-adjacent entry is 13.2.2's `Provisioning: Fix folder rename UID collision during full sync` (#132157) — that is the Git-Sync/full-sync path; our provisioning is file-based and read at boot, so it does not reach us. No alerting changes. **Telegraf 1.40.0** removes `inputs.aerospike`, `inputs.sflow`, `outputs.amon` and deprecated options on kafka_consumer/openstack/procstat/tail — none used in `telegraf.conf`; its `inputs.smart` `power_on_hours` seconds→hours fix applies only to the SATA/SCSI duration-string form and this container's `inputs.smart` is commented out. `inputs.exec`'s value-form deprecation is still un-actioned (removal targeted 1.45.0). **victorialogs and vector did NOT move** — same image id and `.Created` before and after, confirming `pull: always` over the whole stack adopted nothing unreviewed. **Verified 2026-09-17 06:22–06:35Z:** the #95 ping trap did not fire — `getcap /usr/bin/ping` still reports `cap_net_raw=ep` in the new image and the pinger's own OUTPUT is healthy (`ping_result_code`=0 for both targets, 0% loss, 17.3/18.7 ms, 39 unbroken samples across the recreate), not merely "container started". The #94 uid/gid trap did not fire (`uid=999(telegraf) gid=996`), and `docker_*` still carries `host=eq12_docker` across 12 series. Grafana's delivery sentinel `grafana_alerting_alertmanager_receivers` survived the bump — its loss would have fired `obs-alert-delivery-telemetry-absent` on `noDataState: Alerting`. Served surface read from Grafana's own API: **36/36 provisioned rules served, 36/36 evaluating, health `ok` for all 36, both datasources `OK`**. All three telegraf producers (eq12, n5pro, eq12_docker) fresh within 3 min. Rollback path, digest-verified against the running images before the apply: `victoria-metrics:v1.150.0`, `grafana:13.2.0`, `telegraf:1.39.3`. |
| 2026-09-20 | telegraf 1.40.0 → **1.40.0** — a REBUILD, not a version bump (#282) | Watchtower reported `telegraf:latest` `777cdbf33325`. Mapping that digest back through Docker Hub's tag list **before** writing this row showed it also carries the tags **`1.40.0`** and **`1.40`** — the version already running. The delta is the image, not the application: old image built 2026-09-08, new one 2026-09-19, with `TELEGRAF_VERSION=1.40.0` identical in **both image configs** (`docker image inspect`, which is readable without running the superseded image) and the running container reporting `Telegraf 1.40.0 (git: HEAD@e9017dc3)`. So the value here is base-layer patching and there are no release notes to review; a row claiming `1.39.x → 1.40.0` would have been fiction. **Do the digest→tag lookup every time** — a moved floating tag says nothing about whether the app version moved. Unlike #275 the registry had **not** drifted: the notified digest, the local tag and the registry digest were all `777cdbf33325`, checked at 19:25Z and again at 19:27Z immediately before the apply, so what landed is exactly what was notified. **Blast radius checked before applying, not after:** `pull: always` pulls every image in the stack, so all five were compared local-vs-registry first — only telegraf differed, and after the apply `victoriametrics`, `victorialogs`, `vector` and `grafana` had **identical container IDs and `.Created`** (`753957a51211`, `2a415e902d4a`, `88956189583d`, `45e987ba1860`), proving nothing unreviewed rode along. **Verified 2026-09-20 19:28–19:32Z, on telegraf's own OUTPUT rather than on "the container started":** the #95 `no_new_privs` ping trap did not fire — `getcap /usr/bin/ping` still reports `cap_net_raw=ep` and the pinger is producing data (`ping_result_code`=0 for both 1.1.1.1 and 8.8.8.8, 0% packet loss, 17.1/16.8 ms, fresh inside a 90 s lookback); the #94 uid/gid trap did not fire (`uid=999(telegraf) gid=996`); and `host=eq12_docker` is present on **47** `docker_*` metric names / **439** series with **zero** `docker_*` series missing the label (#178) — and because that last one is an absence claim, it was run with a positive control (the same `unless` query against `host="nonexistent-host"` returned all 439, proving the query shape can return results rather than being silently malformed). **The check that covers every plugin at once, and the one to lead with next time:** the set of metric NAMES for `{host="eq12_docker"}` before the recreate (18:30–19:06Z) and after (19:31–19:45Z) is **259 both sides, zero lost, zero new**. That subsumes per-plugin spot checks across all fourteen configured inputs and is the direct answer to #216 — a name that vanishes from an unlabelled aggregate does not announce itself as NoData. Two plugins were then checked individually because repo history says they fail quietly: the `grafana_alerting_alertmanager_receivers` delivery sentinel is still exported (2 series — #275 verified this and #282 nearly dropped the check), and the TrueNAS graphite ingest on `socket_listener :2003` reconnected across the recreate with **21/21 one-minute buckets populated** on `truenas_arcstats_size_size` from 19:20–19:40Z, no gap — worth measuring explicitly because a dead appliance ingest looks exactly like the sparse, legitimately-empty windows the TrueNAS lesson warns about. Served surface unchanged by the recreate: **36/36 provisioned rules served, all health `ok`**, the only firing rule being the deliberate `Alert delivery heartbeat` sentinel, and both datasources report `status=OK` from `/api/datasources/uid/<uid>/health` (the real check — `/api/health` only proves the process is up). One incidental payload change shipped with this apply and is worth not mistaking for a config change: master's `compose.yaml` carried a comment block added by #275 that had never been deployed, so rsync itemized `<fcsT...... compose.yaml`. Comments do not enter compose's service definition and no container config changed. **Rollback is weaker here than for a version bump, and this is the general property of a rebuild:** `c25bff1bb4bf` is still on the host but is now **dangling and unreferenced** (`containers=0`, `RepoDigests=[]`), so the next `docker_prune` (#276/#281) reaps it — that task's comment assumes every rollback is "an immutable upstream tag whose digest was verified against the running image", and for a rebuild **no such tag exists**: `telegraf:1.40.0` now resolves to the NEW image `777cdbf33325`, and `1.39.3` is a downgrade, not this image. Until it is pruned the rollback is the local image ID only. Gap filed as #284. |
