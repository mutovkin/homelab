...
## ZCode mechanics (how this loop maps onto this client)

- **No per-agent model or isolation options.** When this loop says "spawn an
  agent", use the Agent tool with `subagent_type: "general-purpose"` (background
  where the work is long). An agent gets no conversation context — its prompt
  must be fully self-contained (issue text, branch/worktree path, exact
  commands, expected verdicts).
- **Continuing an agent** across fix rounds: `SendMessage` to its `agent_<id>`
  (returned by the Agent call) — it resumes with its prior context intact.
- **Worktrees** are the agent's own job: instruct it to run
  `git worktree add <path> -b fix/<n>-<slug> master` itself; nothing in ZCode
  provisions isolation for you.
- **Plugin skills/commands** are invoked namespaced:
  `compound-engineering:ce-compound`, `superpowers:writing-plans`, etc.
...
ZCode has no plugin-agent registry: spawn each reviewer as a
`subagent_type: "general-purpose"` Agent and **brief it with the persona in its
prompt**. Good personas are ready to reuse in the pr-review-toolkit plugin's
`agents/` directory (under
`~/.zcode/cli/plugins/cache/claude-plugins-official/pr-review-toolkit/0.0.0/agents/`
— `code-reviewer.md`, `silent-failure-hunter.md`, etc.); read the file and fold
its instructions into the spawn prompt. The `/pr-review-toolkit:review-pr`
command works the same way if invoked directly — but it still needs this
persona-briefing adaptation, since its custom agent types don't exist here.
...
Skill("compound-engineering:ce-compound", "mode:headless <one-line context>")
```
or `/compound-engineering:ce-compound <context>` when a human is driving
interactively (it will ask Full-vs-Lightweight and consent questions). Then
commit the new `docs/solutions/...` doc onto the branch as its final commit.
...
- Commit/PR text adds no AI attribution trailer.
