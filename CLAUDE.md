...
- **Reordering PATH on a managed host can move Ansible's interpreter.** Ubuntu 26.04 is
  ...
  `linuxbrew` role (#258) put brew's `python@3` ahead of `/usr/bin` and discovery moved on
  the NEXT run (green, warning only) to an interpreter with no apt bindings. Pin
  `ansible_python_interpreter` in `group_vars/workbench_hosts.yml`, and after any PATH
  change read `discovered_interpreter_python` from `ansible <host> -m ping` on the run
  AFTER the change. Sibling traps (Caskroom, `HOMEBREW_NO_INSTALL_UPGRADE`, cwd, bounded
