...
- #217 — per-host grouping and attribution for this rule and its two siblings, landed:
  all four vector-health rules now group `by (host, ...)`, so the dropout above would be
  a series that stops rather than a member quietly leaving a healthy-looking aggregate.
  That is a change in what is OBSERVABLE, not in what alerts: the rules are
  `noDataState: OK`, so a stopped per-host series still resolves to Normal and pages
  nobody. Nothing yet owns per-host, family-specific absence for
  `vector_buffer_size_bytes` — filed as #222.
